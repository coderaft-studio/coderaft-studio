const reviews = [
  ...
  { name: "Rizky Putra",    role: "CEO, Kreasi Digital",       avatar: "RP", color: "#06b6d4",
...
export default function Testimoni() {
  return (
    <section className="py-16 md:py-24" style={{ background: "#06040f" }}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10 md:mb-14">
          <div>
            ...
            <h2 className="font-black text-white" style={{ fontSize: "clamp(2rem,6vw,4rem)", letterSpacing: "-0.03em" }}>
              Kata<br /><span style={{ color: "#a78bfa" }}>Klien Kami</span>
            </h2>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-2xl sm:text-3xl" style={{ color: "#fbbf24" }}>★★★★★</span>
            <div>
              <div className="font-black text-white text-lg sm:text-xl">5.0</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
          {reviews.map((r) => (
            <div key={r.name} className="rounded-2xl p-5 sm:p-7 flex flex-col justify-between transition-all hover:-translate-y-1 relative overflow-hidden"
              style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <div className="absolute top-3 right-5 font-black text-7xl sm:text-8xl leading-none pointer-events-none select-none"
                style={{ color: `${r.color}10`, fontFamily: "serif" }}>&ldquo;</div>
              <div>
                <div className="flex gap-1 mb-4" style={{ color: "#fbbf24" }}>★★★★★</div>
                <p className="text-sm sm:text-base leading-relaxed relative z-10 italic" style={{ color: "rgba(240,244,255,0.65)" }}>
                  &ldquo;{r.text}&rdquo;
                </p>
              </div>
